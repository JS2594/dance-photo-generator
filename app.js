...
const W = 2525, H = 1894, VERSION = '1.3.5';
...
const COURSE_CARD_CHARACTER = {
  lelepop: './public/assets/characters/lelepop/lelepop_01.png?v=1.3.5',
  buttscaler: './public/assets/characters/buttscaler/buttscaler_01.png?v=1.3.5',
  zumba: './public/assets/characters/zumba/zumba_01.png?v=1.3.5',
  camp: './public/assets/characters/zumba-camp/zumba_camp_01.png?v=1.3.5'
};
...
function load(path){
  ...
  loaded[path] = new Promise((resolve,reject)=>{
    ...
    im.src=inlineSvg
      ...
      : (path.includes('?')?path:path+'?v=1.3.5');
  });
  return loaded[path];
}
...
let POPSHOT_COMBO_CONFIG=null;
async function getCanonicalComboConfig(){
  if(POPSHOT_COMBO_CONFIG)return POPSHOT_COMBO_CONFIG;
  const r=await fetch(`public/custom-combos/custom-combos.json?v=1.3.5`,{cache:'no-store'});
  if(!r.ok)throw new Error(`固定搭配清单加载失败 ${r.status}`);
  POPSHOT_COMBO_CONFIG=await r.json();
  return POPSHOT_COMBO_CONFIG;
}
function comboListFromConfig(data,c){
  const keys={
    lelepop:['lelepop','Lelepop','LELEPOP'],
    buttscaler:['buttscaler','ButtScaler','BUTTSCALER'],
    zumba:['zumba','ZUMBA'],
    camp:['camp','zumba_camp','ZUMBA_CAMP','ZUMBA CAMP']
  }[c]||[c];
  for(const k of keys){
    const v=data?.[k];
    if(Array.isArray(v))return v.filter(Boolean);
  }
  // Support object schema: {courses:{lelepop:[...]}}
  for(const k of keys){
    const v=data?.courses?.[k];
    if(Array.isArray(v))return v.filter(Boolean);
  }
  return [];
}
...
async function renderLosslessExport(){
  let ai=null;
  if(AIHD_ENABLED&&photo){
    try{showSaveToast('AI超清处理中 · 首次需下载约2.6MB模型…');ai=await makeAIHDPhotoCanvas()}
    catch(e){AIHD_LAST_ERROR=String(e);console.warn('AI-HD fallback',e);showSaveToast('AI超清不可用，已自动回退普通高清')}
  }
...
function repairCourseImages(){
  const fallbacks={
    ...
    'buttscaler':'./public/assets/characters/buttscaler/buttscaler_01.png?v=1.3.5',
    'zumba':'./public/assets/characters/zumba/zumba_01.png?v=1.3.5',
    'zumba-camp':'./public/assets/characters/zumba-camp/zumba_camp_01.png?v=1.3.5'
  };
