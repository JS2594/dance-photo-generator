...
// V1.3.5 A-RESTORE: authoritative course-card character images.
// These are sourced from the user-designated canonical character library.
const COURSE_CARD_CHARACTER = {
  lelepop: './public/assets/characters/lelepop/lelepop_01.png?v=1.4.0',
  buttscaler: './public/assets/characters/buttscaler/buttscaler_01.png?v=1.4.0',
  zumba: './public/assets/characters/zumba/zumba_01.png?v=1.4.0',
  'zumba-camp': './public/assets/characters/zumba-camp/zumba_camp_01.png?v=1.4.0'
};
...
function load(path){
  ...
  loaded[path] = new Promise((resolve,reject)=>{
    ...
    im.src=inlineSvg
      ...
      : (path.includes('?')?path:path+'?v=1.4.0');
  });
  return loaded[path];
}
...
async function detectPeople(img){
  ...
  // V1.3.5：本地 pico 优先，等待窗口 1.8s → 4.2s（低端机上 1.8s 常来不及跑完级联，
...
function smartCrop(){
  ...
  const cx=(x1+x2)/2;
  // V1.4.0 TARGET-3 horizontal calibration:
  // move source crop window ~4.5% to the right -> the group moves left in the final frame.
  // Keep zoom and vertical framing unchanged because V1.3.5 subject scale was already acceptable.
  let sx=cx-sw/2 + sw*.045 - photoDX*iw/W;
  // V1.3.5：竖直改为"头顶锚定"——最高人头位于成片高度的 37.5% 处
...
function applyTarget3ExactPhotoPass(targetCtx,w,h,personZones,upscale){
  try{
    ...
    const mb=document.createElement('canvas');mb.width=w;mb.height=h;
    const mg=mb.getContext('2d',{alpha:false,willReadFrequently:true});mg.filter='blur(2px)';mg.drawImage(targetCtx.canvas,0,0);mg.filter='none';
    const md=mg.getImageData(0,0,w,h).data;
    for(let i=0;i<fd.length;i+=4)for(let c=0;c<3;c++){let hp=Math.max(-9,Math.min(9,fd[i+c]-md[i+c]));fd[i+c]=Math.max(0,Math.min(255,fd[i+c]+hp*.18));}
    targetCtx.putImageData(fin,0,0);
  }catch(e){console.warn('Target3 Neutral HD pass skipped',e)}
}


// ── V1.3.5 AI-HD experiment ─────────────────────────────────────────────
let AIHD_ENABLED=false;
...
async function ensureAIHD(){
 if(AIHD_MODEL)return AIHD_MODEL;if(AIHD_LOADING)return AIHD_LOADING;
 AIHD_LOADING=(async()=>{
  await loadScriptOnce('https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@4.22.0/dist/tf.min.js','popshot-tfjs');
  try{await loadScriptOnce('https://cdn.jsdelivr.net/npm/@tensorflow/tfjs-backend-webgpu@4.22.0/dist/tf-backend-webgpu.min.js','popshot-webgpu')}catch(e){}
  try{if(navigator.gpu){await tf.setBackend('webgpu')}else await tf.setBackend('webgl')}catch(e){await tf.setBackend('webgl')}
  await tf.ready();AIHD_MODEL=await tf.loadGraphModel(AIHD_MODEL_URL);return AIHD_MODEL;
 })().catch(e=>{AIHD_LAST_ERROR=String(e);AIHD_LOADING=null;throw e});
 return AIHD_LOADING;
}
async function aiHdUpscaleCanvas(inputCanvas){
 const model=await ensureAIHD(),tile=64,scale=2,overlap=8;
 // V1.3.5: no pre-downscale. AI consumes the original Target3 crop in tiles.
 let work=inputCanvas; // original crop pixels preserved; no precompression/no pre-downscale
 ...
 for(let y=0;y<h;y+=step)for(let x=0;x<w;x+=step){
  const tw=Math.min(tile,w-x),th=Math.min(tile,h-y),tc=document.createElement('canvas');tc.width=tile;tc.height=tile;
  const tg=tc.getContext('2d',{alpha:false});tg.drawImage(work,x,y,tw,th,0,0,tw,th);
  if(tw<tile)tg.drawImage(tc,tw-1,0,1,th,tw,0,tile-tw,th);
  if(th<tile)tg.drawImage(tc,0,th-1,tile,1,0,th,tile,tile-th);
  const inp=tf.browser.fromPixels(tc).toFloat().div(255).expandDims(0);
  let pred=model.predict(inp);if(Array.isArray(pred))pred=pred[0];
  const clipped=tf.tidy(()=>pred.squeeze().clipByValue(0,1)),pix=await tf.browser.toPixels(clipped);
  inp.dispose();pred.dispose();clipped.dispose();
  const pc=document.createElement('canvas');pc.width=128;pc.height=128;
  pc.getContext('2d').putImageData(new ImageData(new Uint8ClampedArray(pix),128,128),0,0);
  const L=x?overlap:0,T=y?overlap:0,R=(x+tw<w)?overlap:0,B=(y+th<h)?overlap:0;
  const sw=tw*2-L-R,sh=th*2-T-B;
  if(sw>0&&sh>0)og.drawImage(pc,L,T,sw,sh,x*2+L,y*2+T,sw,sh);
  await new Promise(r=>setTimeout(r,0));
 }
 return out;
}
async function makeAIHDPhotoCanvas(){
 const c=smartCrop(),base=document.createElement('canvas');
 base.width=Math.max(4,Math.round(c.sw));base.height=Math.max(3,Math.round(c.sh));
 const g=base.getContext('2d',{alpha:false});g.imageSmoothingEnabled=true;g.imageSmoothingQuality='high';
 g.drawImage(photo,c.sx,c.sy,c.sw,c.sh,0,0,base.width,base.height); // direct original-photo crop
 return await aiHdUpscaleCanvas(base);
}
...
async function loadCustomComboConfig(){
  ...
  try{
    ...
    CUSTOM_COMBO_ACTIVE={
      lelepop:arr(src.lelepop||src.Lelepop||src.LELEPOP),
      buttscaler:arr(src.buttscaler||src.ButtScaler||src.BUTTSCALER),
      zumba:arr(src.zumba||src.ZUMBA),
      'zumba-camp':arr(src['zumba-camp']||src.zumba_camp||src.ZUMBA_CAMP||src['ZUMBA CAMP'])
    };
    CUSTOM_COMBO_CONFIG_LOADED=true;
  }catch(e){
    console.error('固定搭配清单加载失败',e);
    CUSTOM_COMBO_ACTIVE={
...
function comboPath(c,name){const dir=(c==='camp'?'zumba-camp':c);return `./public/custom-combos/${dir}/${name}`;}
...
function preloadCourseCombos(c){
  const names=CUSTOM_COMBO_ACTIVE[(c==='camp'?'zumba-camp':c)]||[];
...
async function listCustomCombos(targetCourse=course){
  const names=CUSTOM_COMBO_ACTIVE[(targetCourse==='camp'?'zumba-camp':targetCourse)]||[];
...
async function loadStrictDefaultCombo(targetCourse=course){
  await loadCustomComboConfig();
  const c=(targetCourse==='camp'?'zumba-camp':targetCourse);
  const preferred=DEFAULT_COMBO_FILE[c];
  const manifestNames=CUSTOM_COMBO_ACTIVE[(c==='camp'?'zumba-camp':c)]||[];
  const candidates=[preferred,...manifestNames].filter(Boolean).filter((v,i,a)=>a.indexOf(v)===i);
  for(const filename of candidates){
    const src=comboPath(c,filename);
    const im=await Promise.race([loadComboImage(src),new Promise(r=>setTimeout(()=>r(null),2600))]);
    if(im){
      customComboSrc=src;
      customComboImage=im;
      comboMode='finished';
      ...
      localStorage.setItem('popshotLastCombo_'+c,src);
      preloadCourseCombos(c);
      return true;
    }
  }
...
async function nextCustomCombo(){
  const names=CUSTOM_COMBO_ACTIVE[(course==='camp'?'zumba-camp':course)]||[];
...
input.onchange=e=>{
  ...
  im.onload=async()=>{
    ...
    Promise.race([
      ...
      if(seq!==photoLoadSeq)return;
      if(ok)render();
      if(status)status.textContent=`原图 ${sourceW}×${sourceH} · ${ok?'默认搭配已加载':'默认搭配加载失败，请刷新页面'}`;
      updateCheck();
    }).catch(()=>{});
...
$('#courseGrid').onclick=e=>{
  ...
  Promise.race([
    ...
    $('#detectStatus').textContent=ok
      ...
      : `${courseNames[course]||course} · 默认搭配加载失败`;
    if(!ok)toast((courseNames[course]||course)+' 默认搭配加载失败，请刷新素材');
  }).catch(err=>{
...
$('#generateBtn').onclick=async()=>{
  ...
  try{
    ...
    else{comboMode='custom';layers.title.visible=true;layers.character.visible=true;toast('默认搭配加载失败，已切换到自定义搭配');}
...
async function renderLosslessExport(){
  const ai=null; // V1.4.0 FINAL WEB: no AI upscaling in browser; preserve original crop pixels.
  ...
  canvas=exportCanvas;ctx=exportCtx;POPSHOT_EXPORT_RENDERING=true;AIHD_EXPORT_SOURCE=ai;
  ctx.setTransform(q.w/W,0,0,q.h/H,0,0);
  try{await render()}finally{POPSHOT_EXPORT_RENDERING=false;AIHD_EXPORT_SOURCE=null;canvas=previewCanvas;ctx=previewCtx}
  return {canvas:exportCanvas,w:q.w,h:q.h,ai:!!ai};
}
...
$('#exportBtn').onclick=async()=>{
  ...
  try{
    ...
    console.info(`[PopShot] ${hi.ai?'AI-HD':'HD'} PNG ${hi.w}x${hi.h}, ${mb} MB`);
...
document.addEventListener('DOMContentLoaded',()=>{AIHD_ENABLED=false;localStorage.removeItem('popshot-aihd');});

document.addEventListener('DOMContentLoaded',()=>{loadCustomComboConfig().then(()=>{try{preloadCourseCombos(course)}catch(e){}}).catch(()=>{})});


function applyCanonicalCourseCharacters(){
  const aliases={lelepop:'lelepop',buttscaler:'buttscaler',zumba:'zumba',camp:'camp','zumba camp':'camp','zumba_camp':'camp'};
  document.querySelectorAll('[data-course]').forEach(el=>{
    const raw=String(el.dataset.course||'').toLowerCase();
    const key=aliases[raw]||raw;
    const img=el.querySelector('img');
    if(img&&COURSE_CARD_CHARACTER[key]){
      img.src=COURSE_CARD_CHARACTER[key];
      img.onerror=()=>{console.error('Course character missing',key,img.src)};
    }
  });
  // Fallback for static course cards without data-course: match visible labels.
  document.querySelectorAll('img').forEach(img=>{
    const holder=img.closest('button,div');
    const txt=(holder?.textContent||'').toLowerCase();
    let key=null;
    if(txt.includes('zumba camp'))key='camp';
    else if(txt.includes('buttscaler'))key='buttscaler';
    else if(txt.includes('lelepop'))key='lelepop';
    else if(txt.includes('zumba'))key='zumba';
    if(key&&COURSE_CARD_CHARACTER[key] && /character|avatar|course|thumb|card|assets/i.test(img.src+img.className)){
      img.src=COURSE_CARD_CHARACTER[key];
    }
  });
}

document.addEventListener('DOMContentLoaded',()=>{applyCanonicalCourseCharacters();setTimeout(applyCanonicalCourseCharacters,400)});

async function applyCanonicalDefaultCombo(targetCourse,force=false){
  try{
    const names=await listCustomCombos(targetCourse);
    if(!names.length)throw new Error('该课程固定搭配清单为空');
    const key='popshotLastCombo_'+targetCourse;
    let saved=!force?localStorage.getItem(key):null;
    let chosen=saved && names.includes(saved)?saved:names[0];
    const src=comboPath(targetCourse,chosen);
    const img=await loadComboImage(src);
    if(!img)throw new Error('固定搭配图片加载失败');
    if(typeof comboImg!=='undefined')comboImg=img;
    if(typeof currentComboSrc!=='undefined')currentComboSrc=src;
    localStorage.setItem(key,chosen);
    return true;
  }catch(e){
    console.error('默认固定搭配恢复失败',targetCourse,e);
    return false;
  }
}
